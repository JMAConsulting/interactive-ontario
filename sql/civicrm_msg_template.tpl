...
{php}
  $ogNames = array(
    'claim'         => ts('Message Template Workflow for Claims',          array('escape' => 'sql')),
  );
  $ovNames = array(
    'claim' => array(
      'claim_email' => ts('Claimed Email', array('escape' => 'sql')),
    ),
  );
...
{foreach from=$ovs key=vName item=title name=for_values}
      {fetch assign=subject file="`$config->extensionsDir`biz.jmaconsulting.idm/sql/message_templates/`$vName`_subject.tpl"}
      {fetch assign=text    file="`$config->extensionsDir`biz.jmaconsulting.idm/sql/message_templates/`$vName`_text.tpl"}
      {fetch assign=html    file="`$config->extensionsDir`biz.jmaconsulting.idm/sql/message_templates/`$vName`_html.tpl"}
